...
const DropdownNotification = () => {
  ...
  const Send = async () => {
    ...
    if (receiverId == MyId) {
      dispatch(
        showNotification({
          message: "Except Yourself Input Other Receiver",
          status: 2,
        })
      );
    } else if (user.money >= quantity) {
      ...
      if (res?.data?.msg == "Success!") {
        ...
        window.socket.emit("C2S_CHARGEOFCOIN_SUCCESS_MESSAGE", {
          id: user?.id,
          user: user?.lastName,
          email: user?.email,
          price: quantity,
          description: "chargeCoint",
          receiver: receiverId,
        });
      } else {
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        className={`absolute z-50 -left-[0px] sm:-left-[65px] mt-2 flex overflow-auto w-72 sm:w-96 flex-col rounded-sm border border-stroke bg-white shadow-default pl-6 pr-6 pb-10 ${
      ...
      >
        ...
        <ul className="flex h-auto flex-col overflow-y-auto px-1">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <select
              className="input input-bordered border-[1px] h-[30px] w-56  sm:w-60 focus:outline-none focus:border-minionBlue font-satoshi text-lg"
          ...
          <div className="flex  flex-col sm:flex-row  gap-3">
            ...
            <input
              type="number"
              min={0}
              className="input input-bordered border-[1px] h-[30px] w-56 sm:w-60 focus:outline-none focus:border-minionBlue font-sans"
          ...
          <div className="flex gap-3">
            ...
            <p className="input input-bordered h-[30px] dark:bg-base-100 dark:border-slate-600  w-60 focus:outline-none focus:border-minionBlue font-sans">
              ${user?.money}
            </p>
          </div>
