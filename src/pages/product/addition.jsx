...
export default function Addition() {
  ...
  const handleForm = async (e) => {
    ...
    if (prdData?.name?.trim() === "") {
      dispatch(
        showNotification({ message: "Name field is required!", status: 0 })
      );
    } else if (!prdData?.category || prdData?.category?.trim() === "") {
      dispatch(
        showNotification({ message: "Category field is required!", status: 0 })
      );
      ...
      dispatch(
        showNotification({ message: "Price field is required!", status: 0 })
      );
    } else if (prdData?.quantity.length === 0) {
      dispatch(
        showNotification({ message: "Quantity field is required!", status: 0 })
      );
    } else if (prdData?.discount.length === 0) {
      dispatch(
        showNotification({ message: "Discount field is required!", status: 0 })
      );
    } else {
      setIsLoading(true);
      let fileIdList = [];
      ...
      try {
        ...
        if (res.data.error) {
          return dispatch(
            showNotification({ message: res.data.message, status: 2 })
          );
        } else {
          navigate("../");
          return dispatch(
