...
export default function View() {
  ...
  const sendComment = async () => {
    if(content===""){
      dispatch(showNotification({message:"Input content!", status:0}))
    }
    else{
  ...
  const addCart = async () => {
    if ((productQuantity > prdData?.remain)||productQuantity === 0) {
      dispatch(
      ...
      try {
        const res = await axios.post(
          process.env.REACT_APP_API_BASE_URL + "/api/product/addAProduct",
          {
            _id: prdData?.id,
        ...
        if (res?.data?.type === "success") {
          dispatch(
            showNotification({ message: res?.data?.message, status: 1 })
          );
          dispatch(getACart())
        } else {
          dispatch(
            showNotification({ message: res?.data?.message, status: 0 })
          );
        }
      } catch (err) {}
          ...
          <div className="flex justify-center xl:justify-start gap-12 mt-8">
            <div>
              ...
              <div className="w-full flex justify-center">
                <button
                 ...
                 gap-2 mt-6 border-minionBlue border   py-1 rounded-md px-4 text-xl text-center bg-minionBlue text-white hover:bg-white hover:text-minionBlue duration-500"
                >
            ...
            <div className="xl:order-2 mt-4 order-3 flex h-fit  w-1/2 xl:w-[20%] justify-center flex-col gap-y-6">
              <p className="text-[26px] mt-4 text-minionBlue font-[900]">
                {(prdData?.price * productQuantity).toFixed(1)}
              </p>
