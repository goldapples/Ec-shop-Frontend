...
import { FaCalendar, FaCoins, FaStore } from "react-icons/fa";
...
export default function Dashboard() {
  ...
  const dateChart = () => {
    ...
    for (let i = 0; i <= newMonthDate[1]?.diff(newMonthDate[0], "day"); i++) {
      ...
      allData.push({
        date: newMonthDate[0].add(i, "d").format("YYYY-MM-DD"),
        price: 0,
      });
    }
        ...
        <div className=" mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xxl:grid-cols-4 mt-8 items-center gap-4 ">
            <Cart
              ...
              total={allInfo?.product?.totalProduct}
              current={allInfo?.product?.currentProduct}
              state={1}
            />
            <Cart
              title={"Product Transaction"}
              total={allInfo?.ProductTran?.totalTran}
              current={allInfo?.ProductTran?.currentTran}
              state={2}
            />
            <Cart
              title={"Product Profit"}
              total={allInfo?.profit?.totalProfit}
              current={allInfo?.profit?.currentProfit}
              state={4}
            />
            <Cart
              title={"All Users"}
              total={allInfo?.user?.totalUser}
              current={allInfo?.user?.currentUser}
              state={3}
            />
          </div>
                ...
                <div className="mb-8 flex flex-col gap-3 md:flex-row justify-between items-center">
                  ...
                  <div className="flex items-center gap-3">
                    <label
                      className={`${
                        swicthFlag === true
                          ? "bg-minionBlue text-white"
                          : " bg-gray-200  text-gray-600"
                      } text-xl px-4 py-1 rounded-md`}
                      htmlFor="chart"
                    >
                      Chart
                    </label>
