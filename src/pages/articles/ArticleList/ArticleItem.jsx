...
function ArticleItem({ article, index }) {
  ...
  return (
    <>
      <div
        className={` bg-white flex flex-row gap-x-8 shadow-md rounded-md px-4 w-full h-[200px] text-black hover:bg-slate-200 view mb-3 min-w-[800px]`}
      >
        <div className="flex items-center justify-between flex-col w-[180px] p-2 flex-shrink-0">
          ...
          >
            <img
              src={
                `${process.env.REACT_APP_API_BASE_URL}/api/file/download/${article?.user?.avatar[0]}` ||
                "/image/avatar.png"
              }
        ...
        <div className="flex flex-col items-start justify-between w-[calc(100%-220px)]">
          ...
          >
            <div className="flex flex-col w-full ">
              ...
              <div
                className="text-md font-impact mt-2 truncate w-[80%] text-black inline-block h-[50px] line-clamp-2 description"
                dangerouslySetInnerHTML={{ __html: article?.description }}
              ></div>
              <div className="sm:h-10 md:h-16 flex mt-3">
                {article.files.map((file, index, key) => {
                  return (
                    <Image
                      key={index}
