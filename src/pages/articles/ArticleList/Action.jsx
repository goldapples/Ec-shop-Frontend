...
function Action({ selectedArticle, user, getArticles }) {
  ...
  const addlike = async (data) => {
    setAction(true);
    await window.socket.emit("C2S_ADDLIKE", data);
  };

  const addUnlike = async (data) => {
    setAction(true);
    await window.socket.emit("C2S_UNLIKE", data);
  };
  ...
  useEffect(() => {
    setAction(false);
    window.socket.emit("C2S_GET", selectedArticle?._id);
  }, [action]);

  window.socket.on("S2C_GET", (data) => {
    if (data._id === selectedArticle?._id) {
      setLike(data.like.length);
      setUnlike(data.unlike.length);
    }
  });
  ...
  return (
    <>
      <div className="flex flex-col lg:flex-row">
        <div className="flex items-center justify-center flex-col lg:justify-between w-full  lg:w-[100px] p-2 flex-shrink-0 h-fit">
          <Tooltip
        ...
        <div className=" bg-white flex shadow-md rounded-md px-6 py-2 w-full  text-black mt-3">
          <div className="flex flex-col items-start justify-between w-[calc(100%)] pl-20">
            <div
              className={`flex flex-col w-full ${editorState && "pr-16 pl-8"}`}
            >
              ...
              {selectedArticle?.description ? (
                editorState ? (
                  ...
                  <div
                    className="text-md font-impact mt-3 mb-8 w-[90%] text-black break-all"
