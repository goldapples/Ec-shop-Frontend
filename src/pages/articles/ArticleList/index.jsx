...
import { FaFilter, FaNewspaper, FaSearch, FaUser } from "react-icons/fa";
...
function ArticleList() {
  ...
  const setData = (data) => {
    ...
    if (query.new) {
      filterData = articles
        .filter((article) => article._id === article.ancestorId)
        .filter(
          (article) =>
            article?.user._id !== user._id &&
  ...
  return (
    <>
      <div
        ...
        }  h-[60px] bg-slate-200 duration-500 flex items-center justify-end z-50 overflow-hidden`}
      >
        <div>
          <button
            className="absolute flex items-center justify-center float-right gap-2 top-4 px-2 py-1 hover:text-minionBlue hover:bg-white border-[1px] 
                        hover:border-minionBlue rounded-md text-white bg-minionBlue duration-300 left-[100px] overflow-hidden unreads"
      ...
      <div className="w-full  px-4 h-[calc(100%)] bg-slate-200">
        {searchedData.length > 0 ? (
          searchedData?.map((article, index) => {
            return <ArticleItem article={article} key={index} index={index} />;
          })
        ) : (
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} />
        )}
        <div className="pagination fixed bottom-0 bg-slate-200 mt-[-15px] w-full lg:pr-[250px] sm:pr-[50px] flex justify-center py-2">
          <Pagination
